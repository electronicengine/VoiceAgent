...
AgentTurnResult AgentOrchestrator::RunTurn(
    ...
    std::string currentPrompt = userText;
    AgentTurnResult result;

    for (int step = 0; step < maxAgentSteps_; ++step) {
        const InterpreterResponse response = interpreter_.Interpret(currentPrompt, onPartialResponse);
        if (response.Empty()) {
        ...
        currentPrompt = FormatToolResultMessage(call, toolResult);
    }

    throw std::runtime_error("Agent step limit reached before producing a final response.");
}

ToolCall AgentOrchestrator::ParseToolCall(const InterpreterResponse& response) const {
...
std::string AgentOrchestrator::FormatToolResultMessage(const ToolCall& call, const ToolResult& result) const {
