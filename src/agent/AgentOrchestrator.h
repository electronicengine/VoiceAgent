...
public:
    AgentOrchestrator(IInterpreter& interpreter, const std::vector<ITool*>& tools, const AppConfig& config);
...
private:
    ...
    std::string FormatToolResultMessage(const ToolCall& call, const ToolResult& result) const;
