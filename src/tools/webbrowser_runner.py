...
async def execute_step(page, step, index, artifacts_dir):
    ...
    if action in ("goto", "navigate"):
        ...
        loc = resolve_locator(page, step)
        # use .first to avoid strict-mode failures when selector matches many elements
        await loc.first.wait_for(state=step.get("state", "visible"), timeout=step.get("timeoutMs"))
